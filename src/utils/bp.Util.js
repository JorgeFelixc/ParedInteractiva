...
import {
     ...
     TRY_RESNET_BUTTON_TEXT, updateTryResNetButtonDatGuiCss} from './bp.Example';
import * as partColorScales from './bp.Colors';
 ...
 *
 */
async function setupCamera(cameraLabel, videoDom) {
  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
    throw new Error(
        'Browser API navigator.mediaDevices.getUserMedia not available');
  }

  const videoElement = document.getElementById(videoDom);
...
export async function loadVideo(cameraLabel, videoDom = 'video') {
  try {
    state.video = await setupCamera(cameraLabel, videoDom);
  } catch (e) {
